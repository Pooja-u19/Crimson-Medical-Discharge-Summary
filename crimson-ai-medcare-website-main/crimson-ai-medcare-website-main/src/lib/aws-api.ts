    ...
    if (summary && typeof summary === 'object') {
      ...
      const getValue = (field: any): string => {
        ...
        if (field && typeof field === 'object') {
          if (field.summarizedText) return field.summarizedText;
...
PATIENT INFORMATION
---------------------------------------------------------------
Patient Name      : ${getValue(summaryObj.patientName)}
Age               : ${getValue(summaryObj.age)} years
Gender            : ${getValue(summaryObj.gender)}
IP Number         : ${getValue(summaryObj.ipNo)}
Admission Date    : ${getValue(summaryObj.admissionDate)}
Discharge Date    : ${getValue(summaryObj.dischargeDate)}
Admitting Doctor  : ${getValue(summaryObj.admittingDoctor)}

CLINICAL SUMMARY
---------------------------------------------------------------

PRIMARY DIAGNOSIS:
${getValue(summaryObj.diagnosis)}

PRESENTING COMPLAINTS:
${getValue(summaryObj.presentingComplaints)}

PAST MEDICAL HISTORY:
${getValue(summaryObj.pastHistory)}

SYSTEMIC EXAMINATION:
${getValue(summaryObj.systemicExamination)}

KEY INVESTIGATIONS:
${getValue(summaryObj.keyInvestigationSummary)}

HOSPITAL COURSE:
${getValue(summaryObj.hospitalCourse)}

TREATMENT DURING HOSPITALIZATION:
${getValue(summaryObj.hospitalizationTreatment)}

DISCHARGE MEDICATIONS:
${getValue(summaryObj.dischargeTreatment)}

DISCHARGE ADVICE:
${getValue(summaryObj.advice)}

PREVENTIVE CARE:
${getValue(summaryObj.preventiveCare)}
